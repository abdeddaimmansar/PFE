 ...
 if($_SESSION['loggedin']== false)
 {
   header("location: logout.php");
      ...
      if(ISSET($_GET['id_vol']))
      {
           ...
           $cin = $_GET['id_vol'];
           include("Volume.php");
           $student = new Polycope();
          $fetch = $student->getpoly($cin);
          include("Libelle.php");
          $lib = new Libelle();
          $cats =$lib->GetCats();
        ...
        <li class="submenu">
          ...
          <div class="row">
										...
										<div class="row">
											...
											<div class="col-12 col-sm-6">
												<div class="form-group">
													<label>Titre</label>
													<input type="text" name="titre" class="form-control" value="<?php $_SESSION['id_p'] = $fetch["id_vol"]; echo $fetch["titre"]; ?>">
												</div>
											</div>
											<div class="col-12 col-sm-6">
												<div class="form-group">
                          ...
                          <select name="cat" class="form-control">
                            ...
                            <option selected><?php echo $fetch["liblecat"]; ?></option>
                             <?php foreach ($cats as $category) { $_SESSION[$category["liblecat"]] = $category["id_Cat"]; ?>
                                 <option value=""><?php echo $category["liblecat"]; ?></option>
                            <?php } ?>
                            </select>
                        </div>
                      </div>
                      <div class="col-12 col-sm-6">
                        <div class="form-group">
                          <label>image</label>
                        <img width="100" src='<?php echo $fetch["image_v"];$_SESSION['image_poly'] = $fetch["image_v"]; ?>' class="avatar-img rounded-circle" >
                          <input type="file" name="image" class="form-control" >
											...
											<div class="col-12">
												<button type="submit" name="updatepolycope" class="btn btn-primary">Modifier</button>
											</div>
